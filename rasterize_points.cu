	...
	const bool debug)
{
  ...
  return std::make_tuple(rendered, out_color, out_objects, radii, geomBuffer, binningBuffer, imgBuffer, out_depth, out_opaticy, n_touched);
}
	...
	const bool debug) 
{
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dobjects, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dtau);
}
